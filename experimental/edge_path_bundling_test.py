...
full_path = os.path.join(
    ...
    # '../data/glp1-11k43k.kg.json',  # You can change this to any of your JSON files
)
...
print(f"* extracted {len(pos)} positions for drawing")
...
#%% save the bundled edges to a json file

# merge the bundled edges back to the original graph
data['edges'] = bundled_edges
...
for edge in data['edges']:
    if 'points' in edge:
        edge['points'] = [[round(p[0], 4), round(p[1], 4)] for p in edge['points']]
    if 'control_points' in edge:
        edge['control_points'] = [[round(p[0], 4), round(p[1], 4)] for p in edge['control_points']]

# save the data to a json file
output_path = os.path.join(
    os.path.dirname(__file__), 
    '../data/node_and_bundled_edges_1k6k.json'
)

with open(output_path, 'w') as f:
    json.dump(data, f)
print(f"* bundled edges saved to {output_path}")
